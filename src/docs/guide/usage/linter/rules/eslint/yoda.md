...
要求或禁止 “Yoda” 条件。
此规则旨在强制变量与字面量值进行比较时的条件风格保持一致。

### 为什么这不好？

之所以称为 Yoda 条件，是因为条件中的字面量值排在前面，而变量排在后面。例如，下面就是一个 Yoda 条件：
...
之所以称其为 Yoda 条件，是因为它读起来像“如果红色等于颜色”，类似《星球大战》角色尤达的说话方式。再比较一下另一种安排操作数的方式：
...
Yoda 条件的支持者强调，错误地把 `=` 写成 `==` 是不可能的，因为你不能把值赋给字面量。这样做会导致语法错误，并且会让你尽早发现这个错误。因此，在早期编程中，由于当时还没有可用的工具，这种做法非常常见。
Yoda 条件的反对者指出，工具已经让我们成为更好的程序员，因为工具会捕获把 `=` 误写成 `==` 的错误（ESLint 会帮你检查出来）。因此，他们认为，这种模式带来的收益并不足以抵消使用 Yoda 条件时对可读性的影响。
