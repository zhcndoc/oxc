...
每当 value 属性变化时，React Context 及其所有子节点和消费者都会重新渲染。由于每个 JavaScript 对象都具有自己的身份，像对象表达式（`{foo: 'bar'}`）或函数表达式这类内容在每次渲染时都会获得新的身份。这会让上下文认为它获得了一个新对象，并可能导致不必要的重新渲染和意料之外的后果。
